...
> **목적:** 양 플랫폼의 설정 체계·인증·MCP·Agent Skills·도구 시스템·세션 관리 등의 상세 구조 분석 및 통합 활용 가이드
...
9. [Tools - 에이전트 도구 시스템](#9-tools---에이전트-도구-시스템)
10. [MCP - 외부 통합](#10-mcp---외부-통합)
11. [Extensions - 확장 프로그램 및 도구 통합](#11-extensions---확장-프로그램-및-도구-통합)
12. [Agent 아키텍처 및 특수 에이전트](#12-agent-아키텍처-및-특수-에이전트)
13. [세션 및 작업 관리](#13-세션-및-작업-관리)
14. [Gemini CLI와 Google Code Assist Agent Mode](#14-gemini-cli와-google-code-assist-agent-mode)
15. [상호호환성 종합 매핑](#15-상호호환성-종합-매핑)
16. [통합 실전 가이드 및 팁](#16-통합-실전-가이드-및-팁)
17. [개발자를 위한 Gemini CLI & Antigravity Best Practice](#17-개발자를-위한-gemini-cli--antigravity-best-practice)
18. [참고 문헌](#18-참고-문헌)
...
> *   **통합 관리 위치**: 설정을 한곳에 모으기 위해 `.gemini/GEMINI.md` 경로를 사용할 수도 있습니다. 이 경우 `settings.json` 내의 `context.fileName`에 해당 경로를 명시해야 에이전트가 올바르게 로드할 수 있습니다. (16.1 통합 디렉토리 구조 참조)
...
## 5. 전문 용어 및 응답 정책 (Glossary & Tone)
- **Professional Tone**: 모든 답변은 신뢰감을 주는 전문적인 존칭체를 사용하며, 이모지 사용을 지양합니다.
- **Terminology**: 기술 용어 사용 시 사내 표준 용어집을 참조하여 일관된 번역어를 선택합니다 (예: "Commit" -> "커밋", "Deploy" -> "배포").
- **Structure**: 복잡한 기술적 제안은 반드시 **요약(Summary) - 상세(Details) - 결론(Conclusion)** 순서로 구조화하여 답변합니다.

## 6. 시스템 설계 및 시각화 표준 (Design & Visualization)
- **Diagrams**: 시스템 구조나 흐름을 설명할 때는 반드시 **Mermaid** 문법을 활용한 시각적 다이어그램을 포함합니다.
- **Script Headers**: 모든 셸 스크립트나 자동화 도구 상단에는 반드시 **사용법(Usage)**과 **파라미터 설명** 주석을 포함해야 합니다.
- **Error Guide**: API 명세 작성 시, 에러 코드별 **프론트엔드 대응 가이드** 항목을 필수 섹션으로 추가합니다.

## 7. 리소스 관리 및 보안 (Resource & Security)
- **Modularization**: 신규 파일 생성 시 코드 길이가 500라인을 초과할 것으로 예상되면 논리적 단위로 파일을 분할할 것을 제안합니다.
- **Secret Scan**: Git 커밋 전, 에이전트는 작성된 코드에 하드코딩된 API 키나 비밀번호가 포함되어 있는지 자율적으로 검토합니다.
...
**1. 워크플로우 정의 (`.agent/workflows/secure-api.md`)**
```markdown
# 보안 API 개발 워크플로우
...
**2. 규칙 정의 (`.agent/rules/security-standards.md`)**
```markdown
# API 보안 표준 규칙
...
**3. 상호작용 방식**
에이전트가 워크플로우의 2단계(**로직 구현**)를 수행할 때, 동시에 활성화된 보안 규칙을 참조합니다. 따라서 단순히 기능을 만드는 것에 그치지 않고, **"JWT 인증이 포함되어 있는가?"**, **"보안에 취약한 쿼리를 작성하지 않았는가?"** 를 스스로 검토하며 코드를 생성합니다. 결과적으로 워크플로우는 **생산성**을, 규칙은 **품질과 보안**을 담보하게 됩니다.

#### 활용 시나리오 2: "디자인 시스템 기반 UI 컴포넌트 개발 (TDD)"

**1. 워크플로우 정의 (`.agent/workflows/create-ui-component.md`)**
```markdown
# UI 컴포넌트 TDD 워크플로우
...
**2. 규칙 정의 (`.agent/rules/design-system-standards.md`)**
```markdown
# 디자인 시스템 및 품질 규칙
...
**1. 워크플로우 정의 (`.agent/workflows/release.md`)**
```markdown
# 릴리즈 자동화 워크플로우
...
**3. 시너지 효과**
워크플로우가 **"분석하고, 기록하고, 태깅하라"** 는 기계적인 절차를 수행하는 동안, 규칙은 **"어떤 커밋을 뺄 것인가"**, **"노트를 어떤 스타일로 요약할 것인가"** 에 대한 지능적인 필터링과 편집 가이드를 제공합니다. 결과적으로 사용자는 수작업 없이도 전문적인 수준의 릴리즈 문서를 확보하게 됩니다.
...
에이전트 확장을 위한 세 가지 메커니즘의 차이점은 다음과 같습니다.
...
*   **Gemini CLI**:
    ...
    *   **프로젝트:** `.gemini/skills/`
*   **Antigravity**:
    *   **전역:** `~/.gemini/antigravity/skills/` (전용 경로*)
    *   **프로젝트:** `.agent/skills/`
...
단순한 텍스트 기반 스킬로, Git 작업을 자동화하고 표준화합니다.
...
스크립트 리소스를 포함한 고급 스킬로, 도구 실행을 통해 품질을 제어합니다.
...
*   **폴더 구조**:
    ...
    ├── SKILL.md
    └── scripts/
        └── lint_wrapper.sh  # ruff/pylint 자동 실행 래퍼
    ```
*   **SKILL.md 핵심 내용**:
    ```markdown
    ---
...
*   **scripts/lint_wrapper.sh 코드 예시**:
    ...
    if command -v ruff &> /dev/null; then
        ...
        echo "No linter found. Please install ruff or pylint."
        exit 1
    fi
    ```
...
**Tools**는 에이전트가 사용자의 요청을 수행하기 위해 환경(파일 시스템, 셸, 네트워크 등)과 상호작용하는 실제 '수단'입니다. Gemini CLI는 모델이 스스로 판단하여 함수를 호출하는 **Function Calling** 메커니즘을 기반으로 작동합니다.

### 9.1 도구 작동 원리 (The Tool Loop)
에이전트는 단순히 텍스트를 생성하는 것에 그치지 않고, 다음과 같은 루프를 통해 작업을 완수합니다.
1.  **판단**: 사용자의 요청을 분석하여 도구 실행이 필요한지 결정합니다.
2.  **호출**: 필요한 도구의 이름과 인자(Arguments)를 JSON 형태로 생성합니다.
3.  **실행**: CLI 엔진이 실제 환경에서 해당 도구를 실행하고 결과를 수집합니다.
4.  **관찰**: 도구 실행 결과를 모델에게 다시 전달하여 다음 단계(추가 실행 또는 최종 답변)를 결정하게 합니다.

### 9.2 핵심 도구 (Core Tools) 카테고리
Gemini CLI는 설치 시 기본적으로 다음의 핵심 도구 세트를 제공합니다.

| 카테고리 | 주요 도구 명칭 | 설명 |
|----------|----------------|------|
| **File System** | `read_file`, `write_file`, `replace`, `list_directory` | 파일을 읽고 쓰고 수정하며 디렉토리를 탐색합니다. |
| **Search** | `search_file_content` (ripgrep), `glob` | 코드베이스 내에서 특정 패턴이나 파일을 고속 검색합니다. |
| **Shell** | `run_shell_command` | 임의의 Bash 명령어를 실행합니다 (컴파일, 테스트 등). |
| **Memory** | `save_memory` | 사용자의 선호도나 중요한 사실을 장기 기억에 저장합니다. |
| **Specialized** | `codebase_investigator`, `cli_help` | 복잡한 코드 분석이나 도움말 제공을 위한 서브에이전트 도구입니다. |

### 9.3 도구 사용 제어 및 보안 (Security & Control)
에이전트가 임의의 명령을 실행하는 것을 방지하기 위해 `settings.json`에서 엄격한 제어가 가능합니다.

*   **승인 정책 (`autoAccept`)**: `false`로 설정 시 모든 도구 실행 전 사용자에게 승인 확인을 요청합니다.
*   **허용 목록 (`allowed`)**: 특정 명령어 세트만 실행 가능하도록 제한합니다 (예: `run_shell_command(npm test)`).
*   **제외 목록 (`exclude`)**: 에이전트가 절대 사용하지 못하게 할 도구를 지정합니다.
*   **샌드박싱 (`sandbox`)**: 도구가 실제 호스트 시스템이 아닌 Docker나 Podman 컨테이너 내부에서 실행되도록 격리합니다.

### 9.4 도구 호출 실전 예시 (Scenario)
...
// 2. search_file_content 도구로 패턴 검색
tool_call: search_file_content(pattern="TODO", include="**/*.py")
// [결과]: "main.py:42: # TODO: 예외 처리 추가 필요"

// 3. 최종 답변 생성
"검색 결과, main.py의 42번 라인에서 '# TODO: 예외 처리 추가 필요' 주석을 발견했습니다."
```

### 9.5 플랫폼별 호환성 (Platform Compatibility)
도구 시스템은 Gemini 에이전트의 핵심 엔진을 공유하므로 플랫폼 간 높은 호환성을 유지합니다.

*   **핵심 도구 (Core Tools) - ✅ 공통**: `read_file`, `run_shell_command`, `search_file_content` 등의 표준 도구 세트는 **Gemini CLI와 Antigravity에서 동일하게 작동**합니다. 양측 모두 동일한 에이전트 코어 엔진을 기반으로 하기 때문입니다.
*   **플랫폼 특화 도구**:
    - **Antigravity**: IDE 환경을 활용한 `browser_subagent` (실시간 웹 탐색), `vscode_api` 연동 도구 등이 추가로 제공됩니다.
    - **Gemini CLI**: 터미널 환경에 최적화된 PTY(가상 터미널) 제어 및 하위 레벨 셸 상호작용 도구가 포함되어 있습니다.
...
```
*참조: [Sequential Thinking 서버 공식 문서](https://github.com/modelcontextprotocol/servers/tree/main/src/sequentialthinking)*
...
| `cwd` | 서버 실행 디렉토리입니다. |
| `url` | SSE 트랜스포트용 URL입니다. |
| `httpUrl` | Streamable HTTP 트랜스포트용 URL입니다. |
...
#### **OAuth 인증 지원 설정**
원격 MCP 서버에 대해 OAuth 2.0 인증을 구성할 수 있습니다.
...
#### **gemini mcp CLI 명령어 목록**
터미널에서 MCP 서버를 관리할 수 있는 명령어입니다.
...
# 세션 내 MCP 상태 확인 (슬래시 커맨드)
/mcp
```

### 10.2 Antigravity MCP 설정
...
*   하나의 프록시 서버를 Antigravity에 등록하고, 프록시 내부에서 프로젝트 경로에 따라 실제 MCP 서버로 라우팅합니다.
*   이 방식을 통해 프로젝트마다 다른 MCP 도구 세트를 유연하게 사용할 수 있습니다.

> **💡 최적화 팁:** Antigravity 에이전트의 응답 성능을 위해 **활성화된 도구 수를 50개 이하**로 유지하는 것을 권장합니다.
...
## 11. Extensions - 확장 프로그램 및 도구 통합

Extensions는 Gemini CLI의 기능을 확장하기 위한 **네이티브 확장 시스템**입니다. MCP가 별도의 서버 프로세스를 통해 도구를 제공한다면, Extensions는 CLI 환경 내에서 보다 직접적으로 도구를 정의하고 실행할 수 있게 해줍니다.

### 11.1 Extensions vs MCP 차이점
| 구분 | MCP (Model Context Protocol) | Extensions |
|------|-----------------------------|------------|
| **구조** | 외부 서버 (stdio/HTTP) 통신 방식입니다. | CLI 내 도구(Tool) 직접 정의 방식입니다. |
| **복잡도** | 서버 구현이 필요하여 상대적으로 복잡합니다. | JSON 설정과 스크립트만으로 구현 가능합니다. |
| **성능** | 프로세스 간 통신 오버헤드가 발생할 수 있습니다. | 네이티브 실행으로 오버헤드가 최소화됩니다. |
| **용도** | 대규모 도구 세트, SaaS 연동에 적합합니다. | 로컬 자동화, 특정 도구 확장 등에 적합합니다. |

### 11.2 작성 가이드 (Writing Extensions)
확장 프로그램은 `extension.json` 설정 파일과 실행 가능한 스크립트로 구성됩니다.

1.  **구성 요소**:
    - **`extension.json`**: 확장 프로그램의 이름, 버전, 제공하는 도구(Tools)의 스키마를 정의합니다.
    - **실행 스크립트**: 모델이 도구를 호출했을 때 실제로 실행될 로직(Python, Node.js, Shell 등)입니다.

2.  **작성 단계**:
    - `.gemini/extensions/` 디렉토리 내에 확장 프로그램 이름으로 폴더를 생성합니다.
    - `extension.json` 파일을 작성하여 도구의 파라미터와 설명을 상세히 기술합니다.
    - 스크립트 파일에 실행 권한을 부여하고, 입출력을 JSON 형태로 처리하도록 구현합니다.

### 11.3 실전 샘플 예시 (Python 기반 시스템 정보 도구)
시스템의 리소스 정보를 확인하는 간단한 확장 프로그램 예시입니다.
...
```json
{
  "name": "system-monitor",
  "version": "1.0.0",
  "description": "시스템 리소스(CPU, 메모리) 상태를 확인합니다.",
  "tools": [
    {
      "name": "get_system_stats",
      "description": "현재 CPU 및 메모리 사용량을 반환합니다.",
      "parameters": {
        "type": "object",
        "properties": {}
      },
...
### 11.4 베스트 프랙티스 (Best Practices)
*   **보안 (Security)**: 확장 프로그램은 로컬 시스템 권한으로 실행되므로, 입력값에 대한 엄격한 유효성 검사(Input Validation)가 필수적입니다.
*   **오류 처리 (Error Handling)**: 도구 실행 실패 시 모델이 이해할 수 있도록 명확한 에러 메시지를 `stderr` 대신 `stdout`의 JSON 내부에 포함하여 반환하십시오.
*   **입출력 표준화**: 모든 데이터 교환은 **JSON 포맷**을 기본으로 사용하며, 모델이 파싱하기 쉬운 구조로 설계하십시오.
*   **비동기 최적화**: 시간이 오래 걸리는 작업은 진행 상태를 출력하거나 타임아웃 처리를 포함하여 에이전트의 응답 지연을 방지하십시오.

### 11.5 설치 및 배포
*   **로컬 설치**: 작성된 확장 폴더를 `~/.gemini/extensions/` 경로에 복사합니다.
*   **활성화 확인**: CLI 실행 후 `/tools` 명령어를 통해 등록된 도구가 목록에 표시되는지 확인합니다.

### 11.6 Antigravity와의 호환성 (Compatibility)
⚠️ **중요: Gemini CLI의 네이티브 Extensions는 Gemini CLI 독자 기능이며 Antigravity와 직접 호환되지 않습니다.**

*   **독립적 시스템 (CLI 전용)**: CLI의 `extension.json` 기반 시스템은 터미널 환경에 최적화된 **독자 기능**입니다. IDE 환경인 Antigravity에서는 해당 파일을 직접 로드하여 도구로 인식하지 않습니다.
*   **대체 방법**: Antigravity에서 동일한 도구 기능을 사용하려면 **Agent Skills**나 **MCP** 방식을 권장합니다.

---

## 12. Agent 아키텍처 및 특수 에이전트
...
### 12.3 Browser Subagent (Antigravity 전용)
실시간 웹 상호작용 및 문서 탐색을 수행하는 강력한 서브 에이전트입니다.
*   **보안 격리**: 전용 프로필(`antigravity-browser-profile`)을 사용하여 개인정보를 보호합니다.
*   **활용**: 최신 프레임워크 문서 조회, UI 버그 재현 테스트 등에 활용합니다.

---

## 13. 세션 및 작업 관리

### 13.1 세션 보존 및 복구 (Gemini CLI)
...
## 14. Gemini CLI와 Google Code Assist Agent Mode

### 14.1 핵심 관계
**Gemini Code Assist의 Agent Mode(VS Code)는 Gemini CLI를 기반(Backend)으로 구동됩니다.** Google 공식 문서에 따르면, Agent Mode는 Gemini CLI의 강력한 핵심 기능을 IDE 채팅 인터페이스 내에서 **가장 효율적으로 사용할 수 있도록 최적화된 서브셋(Subset)** 형태로 제공합니다. 두 도구는 동일한 에이전트 기술을 공유하므로, 대부분의 설정과 구성 파일이 상호 호환됩니다.

### 14.2 GEMINI.md 호환성
`GEMINI.md`는 Gemini 모델에게 지속적인 지시 컨텍스트를 제공하는 마크다운 파일로, 양측 모두 동일한 **계층적 구조(Hierarchical Context)** 시스템을 완벽히 지원합니다.

| 범위 | 위치 | 설명 |
|------|------|------|
| **전역 (Global)** | `~/.gemini/GEMINI.md` | 모든 프로젝트에 공통으로 적용되는 최상위 규칙입니다. |
| **프로젝트 (Project)** | 프로젝트 루트 또는 상위 경로 | 특정 프로젝트의 도메인 지식이나 아키텍처 규칙을 정의합니다. |
| **서브디렉토리** | 하위 디렉토리 내 `GEMINI.md` | 특정 모듈이나 컴포넌트에 국한된 세부 지침을 적용할 때 사용합니다. |

#### **[플랫폼별 GEMINI.md 기능 지원 현황]**
...
> **💡 통합 활용 팁:** Gemini CLI의 `settings.json`에서 `context.fileName`을 통해 파일명을 커스터마이징하면, VS Code의 Agent Mode 역시 해당 설정값을 참조하여 동일한 파일을 컨텍스트로 자동 인식합니다.

### 14.3 공유 인프라 및 설정
Agent Mode와 Gemini CLI는 기술적 뿌리가 같기 때문에, **동일한 일일 요청 쿼터(Quota)**와 설정 파일을 공유하여 원활한 전환이 가능하도록 설계되었습니다.

*   **공유 쿼터 (Quota)**: 한쪽 플랫폼에서 사용한 요청 횟수가 다른 쪽의 잔여 한도에 즉각 반영됩니다. (Individual: 일 1,000회, Enterprise: 일 2,000회)
*   **공유 설정 (`~/.gemini/settings.json`)**:
    - **MCP 서버 연동**: CLI에서 구성한 MCP 서버를 Agent Mode에서도 별도 설정 없이 즉시 호출할 수 있습니다.
    - **도구 실행 제어**: `coreTools` 허용 및 `excludeTools` 차단 목록이 양쪽 환경에 공통으로 적용됩니다.
    - **Yolo Mode**: 사용자 명시적 승인 없이 도구를 실행하는 자동화 설정이 두 환경 간에 동기화됩니다.

### 14.4 공통점 및 차이점 분석
두 플랫폼은 동일한 ReAct 루프 엔진을 사용하지만, **실행 환경의 특성**에 따라 다음과 같은 기능적 차이를 보입니다.

#### **[핵심 차이점 상세 비교]**
| 비교 항목 | Gemini CLI | Code Assist Agent Mode |
|-----------|-----------|----------------------|
| **실행 환경** | 터미널 기반의 독립적인 실행 환경을 제공합니다. | VS Code 및 IntelliJ IDE 내부에 완벽히 통합되어 있습니다. |
| **기능 범위** | 에이전트의 모든 기능을 활용할 수 있는 **슈퍼셋(Superset)** 입니다. | IDE 작업에 필수적인 기능 위주의 **서브셋(Subset)** 을 제공합니다. |
| **특화 기능** | **PTY 지원, 체크포인팅, 샌드박싱** 등 고도의 시스템 제어가 가능합니다. | **인라인 코드 완성, 시각적 Diff 리뷰** 등 편집 편의성에 집중합니다. |
| **고급 확장성** | **Agent Skills, 토큰 캐싱, 시스템 프롬프트 오버라이드**를 지원합니다. | IDE 워크스페이스 기반의 **자동 인덱싱 및 시맨틱 검색**에 최적화되어 있습니다. |

### 14.5 시나리오별 권장 도구
작업의 성격에 따라 가장 효율적인 도구를 선택하여 생산성을 극대화할 수 있습니다.

*   **Gemini CLI 사용 권장**:
    - **CI/CD 파이프라인 통합**: 별도의 GUI 없이 Headless 환경에서 자동화 작업을 수행할 때 유리합니다.
    - **원격 서버 및 SSH 환경**: 서버 터미널에서 즉각적인 코드 수정이나 인프라 관리가 필요할 때 적합합니다.
    - **고급 커스터마이징**: **Agent Skills** 정의나 복잡한 **샌드박싱** 설정이 필수적인 전문 개발 작업에 권장합니다.
*   **Agent Mode 사용 권장**:
    - **실시간 코딩 보조**: 에디터에서 코드를 작성하며 실시간 대화와 코드 생성을 병행하고 싶을 때 유용합니다.
    - **시각적 코드 리뷰**: 변경 사항을 Diff 뷰로 직접 확인하고 승인하는 직관적인 워크플로우가 중요할 때 사용합니다.

### 14.6 실무적 시사점
두 도구는 서로 배타적인 선택지가 아니며, **동일한 컨텍스트와 MCP 연동 환경** 을 공유하는 강력한 파트너입니다. 

따라서 **터미널에서의 벌크 작업이나 자동화**에는 Gemini CLI를, **IDE 내부에서의 정밀한 로직 구현**에는 Agent Mode를 상황에 맞게 자유롭게 전환하며 활용하는 것이 가장 스마트한 개발 전략입니다.
...
│   ├── .env                    # 프로젝트 전용 인증 (선택 사항)
│   ├── GEMINI.md               # 프로젝트 컨텍스트 및 지침
│   ├── settings.json           # 프로젝트 전용 도구/MCP 설정
│   ├── skills/                 # [원본 Skills 저장소]
│   │   └── deploy-staging/
│   │       └── SKILL.md
│   └── extensions/             # 프로젝트 전용 확장 프로그램
...
## 17. 개발자를 위한 Gemini CLI & Antigravity Best Practice

본 섹션은 동일한 개발 환경에서 Gemini CLI와 Antigravity를 병행 사용하는 개발자를 위해, **“한 번의 설정으로 양 플랫폼의 강점을 극대화하는”** 실전 활용 전략을 정리합니다.

### 17.1 전체 원칙 (Core Principles)

*   **공용 설계**: 컨텍스트(`GEMINI.md`), 전문 지식(`SKILL.md`), 외부 도구(`MCP`), 인증(`.env`)은 최대한 **양 플랫폼이 공유하도록** 설계하여 중복 관리를 배제합니다.
*   **역할 분리**: “복잡한 IDE 작업(코딩, 리팩토링)은 **Antigravity**, 터미널 자동화 및 배포·실험은 **Gemini CLI**”로 역할을 나누되 컨텍스트를 동기화하는 것이 가장 효율적입니다.
*   **IDE 연동**: Gemini CLI의 `/ide` Companion 기능을 활성화하여 Antigravity 내에서 CLI의 강력한 분석 도구를 직접 활용하십시오.

### 17.2 디렉토리·파일 구조 Best Practice

#### **17.2.1 홈 디렉토리 공용 설정 (Global Context)**
사용자 홈의 `.gemini` 폴더를 중앙 설정 저장소로 활용합니다.
...
> **💡 Skills 공유 팁 (심볼릭 링크)**  
> Skill 원본은 `~/.gemini/skills/`에 유지하고, Antigravity용 경로에서는 심볼릭 링크를 통해 참조하는 패턴이 유지 관리에 매우 유리합니다.
> ```bash
> ln -s ~/.gemini/skills/deploy-staging ~/.gemini/antigravity/skills/deploy-staging
> ```

#### **17.2.2 프로젝트별 구조 (Local Context)**
프로젝트 루트 내에서도 플랫폼별 특성을 고려하여 계층을 나눕니다.

| 파일/경로 | 용도 및 관리 전략 |
|-----------|------------------|
| **`.gemini/GEMINI.md`** | **모델이 항상 인지해야 할 핵심 요약**을 작성합니다. (공통 규칙) |
| **`.agent/rules/*.md`** | Antigravity 특유의 **UI 지침, 워크플로우 메모** 등을 상세히 작성합니다. |
| **`.gemini/settings.json`** | 프로젝트 전용 MCP 서버나 도구 허용 목록을 관리합니다. |

### 17.3 Auth / .env / Quota 관리 전략

*   **공용 .env**: `~/.gemini/.env`에 공용 API 키를 설정하면, Gemini CLI는 자동 로드하고 Antigravity는 OS 환경변수를 통해 이를 참조할 수 있습니다.
*   **Quota 관리**: 
    - **Antigravity**: 짧은 대화, 인터랙티브한 코드 편집 및 시각적 수정 위주로 사용합니다.
    - **Gemini CLI**: 대규모 코드베이스 분석(`/investigate`), 복잡한 MCP 작업, 배치성 자동화에 집중합니다.
    - 이를 통해 단일 계정의 백엔드 쿼터를 효율적으로 분배하여 소진을 방지합니다.

### 17.4 GEMINI.md / Rules 통합 전략

*   **단일 진실 공급원 (SSOT)**: `~/.gemini/GEMINI.md`를 최상위 코딩 표준으로 정의하십시오. Antigravity는 이 파일을 자동으로 Rules에 포함하여 인식합니다.
*   **계층적 매핑**: 
    - **`.gemini/GEMINI.md`**: 전역/프로젝트 공통 핵심 요약 정보를 담습니다.
    - **`.agent/rules/`**: IDE 내에서 에이전트에게 내릴 구체적인 행동 지침을 보완합니다.

### 17.5 Skills / Workflows / Commands 설계

*   **Agent Skills 완전 공유**: `SKILL.md`는 오픈 표준이므로 플랫폼 전용 필드를 피하고 순수 마크다운 지시사항 위주로 작성하여 Claude Code, Gemini CLI, Antigravity 어디서든 재사용 가능하게 관리합니다.
*   **특성화 도구**:
    - **Workflows (`.agent/workflows/`)**: IDE 유저 플로우(컴포넌트 생성 등)에 최적화합니다.
    - **Custom Commands (`.gemini/commands/`)**: 터미널/CI 플로우(빌드, 배포 스크립트 실행)에 특화합니다.

### 17.6 MCP 서버 공유 전략

동일한 MCP 서버(예: Filesystem, DB 연동 등)를 양쪽 환경의 설정 파일(`settings.json` vs `mcp_config.json`)에 각각 등록하여 사용하십시오. 서버 구현체는 하나로 유지하고 호출 방식만 플랫폼에 맞춥니다.

### 17.7 Gemini CLI ↔ Antigravity IDE 연동 (Companion)

Antigravity 터미널에서 `/ide install` 및 `/ide enable` 명령을 통해 연동을 활성화하십시오.
*   CLI에서 제안한 변경 사항을 Antigravity의 **네이티브 Diff 뷰**로 확인하고 승인할 수 있습니다.
*   복잡한 분석은 CLI에서 수행하고, 실제 코드 반영은 IDE의 편리한 UI를 활용하는 하이브리드 워크플로우가 가능해집니다.

### 17.8 실전 워크플로우 시나리오

1.  **신규 기능 개발**: Antigravity Workflow로 골격을 생성하고, Gemini CLI의 `/investigate`로 전체 아키텍처 정합성을 검토한 뒤 Companion으로 코드를 반영합니다.
2.  **버그 분석**: Antigravity에서 에러 로그를 확인하고, Gemini CLI에서 전역 탐색 도구(ripgrep 등)와 MCP를 연동하여 근본 원인을 추적합니다.
3.  **릴리즈 및 배포**: Gemini CLI의 Custom Command로 배포 스크립트를 실행하고, 결과 요약을 `GEMINI.md`에 기록하여 이후 모든 세션에서 릴리즈 정보를 공유하게 합니다.

### 17.9 요약 체크리스트 (Best Practice)

- [ ] `~/.gemini/.env`에 공용 API 키 설정 및 환경변수 노출 확인
- [ ] `~/.gemini/GEMINI.md`에 전역 공통 코딩 표준 정의
- [ ] 프로젝트 규칙을 `.gemini/GEMINI.md`(핵심)와 `.agent/rules/`(상세)로 계층화
- [ ] Agent Skills 원본을 관리하고 각 플랫폼 경로에 심볼릭 링크 생성
- [ ] 공용 MCP 서버를 양쪽 설정 파일에 등록
- [ ] `/ide enable`을 통한 Companion 연동 활성화

---

## 18. 참고 문헌

본 가이드 작성에 참고한 공식 문서 및 기술 자료 목록입니다.
...
*   [Gemini CLI Tools Guide](https://geminicli.com/docs/tools/)
